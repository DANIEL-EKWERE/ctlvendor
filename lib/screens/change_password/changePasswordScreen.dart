...
class _ChangePasswordScreenState extends State<ChangePasswordScreen> {
  ...
  void initState() {
    super.initState();
    controller.newPassword.addListener(_validateForm);
    controller.confirmNewPassword.addListener(_validateForm);
  }

  void _validateForm() {
    // final email = controller.newPassword.text;
    final password = controller.confirmNewPassword.text;
    final passwordConfirm = controller.confirmNewPassword.text;
    // final emailValid = RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(email);
    final passwordValid = password.length >= 8;
    final passwordConfirmValid = passwordConfirm.length >= 8;

    setState(() {
      _isButtonEnabled = passwordConfirmValid && passwordValid;
    });
  }
